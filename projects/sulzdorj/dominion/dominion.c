...
int playBaron( struct gameState *state, int currentPlayer, int choice1 ){
  ...
  if (choice1 > 0){//Boolean true or going to discard an estate
    ...
    /* 
      Bug #1 - changed the initilized value of p from 0 to 1 so that the first card in the player's hand
      is never evaluated. 
     */
    ...
    for (int i = 0; i < state->numPlayers; i++) {
      if (i != currentPlayer) {
        if ( state->handCount[i] > 4 ) {
          ...
          /*
            Bug #2: Limit the for loop to 3 so that each player that's not the currentPlayer
            only gets 3 cards at the end of this function call.  
           */
          //draw 4
          for (int j = 0; j < 3; j++){
            drawCard(i, state);
...
int playAmbassador( struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2 ){
  ...
  /*
    Bug #1: Stop the player from trashing two cards. If they choose to trash this many, 
    the function will return here and indicate they cannot play the Ambassador card.
   */
  ...
  /*
   Bug #2: Instead increasing the supply count for the card that's being discarded 
   by the number of cards the player choose, multiply by that many cards. 
   */
...
int playTribute( struct gameState *state, int currentPlayer, int nextPlayer, int tributeRevealedCards[] ){
  ...
  for (int i = 0; i < 2; i ++){
    ...
    /*
      Bug #2: Only draw one card if the card is a "victory" card
     */
...
int playMine( struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2 ){
  ...
  /*
    Bug 1: If a player tries to swap a copper treasure card, don't let them. 
   */
  ...
  /*
    Bug 2: Remove the check to ensure the player doesn't input a negative value for their choice2.
   */
