...
protected void setUp() {
    ...
    */
   public void testRandomTesting() throws IOException {
       for(int i = 0; i < 2000; i++) {
           ...
           // Build some user info if we want to
           String userInfo = userInfoGen();

           // Generate a port number - this is going to be where our bug is introduced
           // If this number is above 4095, then our call to UrlValidator.isValid() will return false, even though it should return true
           int portNum = random.nextInt(65335); 
         

           // Generate our path
           String path = pathGen();

           // Build the full URL from the calls below
           String URLtoTest = scheme + userInfo + host + ".com:" + portNum + "/" + path;

           // Get our URL Validator instance up and running
           long options = UrlValidator.ALLOW_2_SLASHES + UrlValidator.ALLOW_ALL_SCHEMES + UrlValidator.NO_FRAGMENTS;
           UrlValidator urlVal = new UrlValidator(null, null, options);

           // Check to see if UrlValidator thinks the URL is valid
           boolean isValidResult = urlVal.isValid( URLtoTest );
               
           // Check to see if isValid() returns any false values
           try {
        	   assertTrue(URLtoTest + " failed", isValidResult);   
           } 
           // And if it does, then this catch statement should run and print out the URL
           catch( AssertionError e) {
        	   System.out.println( e );
           }
          
           /*
            * I've found the block below to be helpful in showing valid URLs vs. invalid URLs but it's only useful when debugging
            * Uncomment this out to see the valid URLs generated by the randomizer and demonstrate that these URLs are never printed out
            * as an error
           if( isValidResult == true ) {
        	   System.out.println( "This was a valid URL according to UrlValidator " + URLtoTest );
           }*/
